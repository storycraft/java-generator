...
public class TreeMakerUtil {
    @Nullable
    public static JCTree.JCExpression createClassName(TreeMaker treeMaker, Names names, String... dotSeparated) {
        if (dotSeparated.length < 1) {
            return null;
        }

        JCTree.JCExpression expr = treeMaker.Ident(names.fromString(dotSeparated[0]));
